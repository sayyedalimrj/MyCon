"""Stage 4 configuration helpers.

These helpers intentionally mirror the small dotted-key access pattern used by
previous stages while keeping Stage 4 independent from optional dependencies.
"""
...
class Stage4ConfigError(RuntimeError):
    """Raised when Stage 4 configuration or file contracts are invalid."""
...
def cfg_require(cfg: Any, dotted_key: str) -> Any:
    if hasattr(cfg, "require"):
        try:
            ...
            raise Stage4ConfigError(f"Missing required config key for Stage 4: {dotted_key}") from exc
    value = cfg_get(cfg, dotted_key, None)
    if value is None:
        raise Stage4ConfigError(f"Missing required config key for Stage 4: {dotted_key}")
    return value
...
def cfg_int(cfg: Any, dotted_key: str, default: int) -> int:
    ...
    try:
        ...
        raise Stage4ConfigError(f"Config key {dotted_key} must be an integer, got {value!r}") from exc
...
def cfg_float(cfg: Any, dotted_key: str, default: float) -> float:
    ...
    try:
        ...
        raise Stage4ConfigError(f"Config key {dotted_key} must be a float, got {value!r}") from exc


def project_root(cfg: Any) -> Path:
    root = Path(str(cfg_require(cfg, "project.root")))
    if not root.is_absolute():
        raise Stage4ConfigError(f"project.root must be absolute inside Docker/WSL, got: {root}")
    return root
...
def resolve_project_path(cfg: Any, dotted_key: str, default: str | None = None) -> Path:
    ...
    if raw is None:
        raise Stage4ConfigError(f"Missing path config key for Stage 4: {dotted_key}")
    path = Path(str(raw))
...
def project_name(cfg: Any) -> str:
    ...
    if not value.strip():
        raise Stage4ConfigError("project.name must not be empty")
    return value
...
def run_id(cfg: Any) -> str:
    ...
    if not value.strip():
        raise Stage4ConfigError("project.run_id must not be empty")
    return value
